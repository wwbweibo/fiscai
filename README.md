...
<div>
  <img src="./assets/fiscai1.png" alt="FiscAI" width="200" />
  <img src="./assets/fiscai2.png" alt="FiscAI" width="200" />
</div>
...
- [x] 1. 用户输入：通过chat页面告知AI，AI根据用户输入的账单信息，识别账单信息，并记录到账单中。
- [x] 2. 图片识别：通过拍照，识别账单信息，并记录到账单中。
- [x] 3. 语音识别：通过语音识别，识别账单信息，并记录到账单中。
- [ ] 4. 系统通知：如果授权了读取系统通知权限，则可以自动识别系统通知中的账单信息，并记录到账单中。
- [x] 5. 截屏识别：如果授权了监听截屏权限，则截屏后询问用户是否需要识别截屏中的账单信息，并记录到账单中。（Android 端实现）

#### 账单分析

支持多维度账单分析：

- [x] 1. 账单分类：根据账单信息，自动分类到不同的账单分类中。
- [x] 2. 账单统计：根据账单信息，自动统计到不同的账单统计中。
- [x] 3. 账单分析：根据账单信息，自动分析到不同的账单分析中。
...
启动APP，在左上角点击设置按钮，进入设置页面，配置你的OpenAI API密钥。